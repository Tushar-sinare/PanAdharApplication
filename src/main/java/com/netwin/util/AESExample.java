...
static final MyLogger logger = LoggerProvider.getLogger(AESExample.class);
    ...
    // Encrypt function
	/*
	 * public static String encrypt(String plainText, String key){ String
	 * result=null; try { byte[] iv = generateRandomIV(); SecretKey secretKey = new
	 ...
	 * 
	 * result= Base64.getEncoder().encodeToString(combined); } catch (Exception e) {
	 * result= "Encryption failed: " + e.getMessage(); } logger.info(result); return
	 * result; }
	 * 
	 * // Decrypt function public static String decrypt(String encryptedText, String
	 * key){ String result=null; try { byte[] combined =
	 * Base64.getDecoder().decode(encryptedText); byte[] iv = new
	 * byte[CBC_IV_LENGTH]; byte[] encryptedBytes = new byte[combined.length -
	 * CBC_IV_LENGTH];
	 * 
	 * System.arraycopy(combined, 0, iv, 0, CBC_IV_LENGTH);
	 * System.arraycopy(combined, CBC_IV_LENGTH, encryptedBytes, 0,
	 * encryptedBytes.length);
	 * 
	 ...
	 * 
	 * result= new String(decryptedBytes); } catch (Exception e) {
	 * 
	 * result = "Decryption failed: " + e.getMessage(); } return result; }
	 */
	  private static byte[] generateRandomIV() { byte[] iv = new
